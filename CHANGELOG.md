...
* [CRuby] `Document#canonicalize` snapshots its inclusive namespace prefixes, so coercing a later prefix or compacting the heap from the visibility block can't move or change an earlier one.
* [CRuby] `Document#canonicalize` releases its native canonicalization state when the visibility block raises or throws.
